...
stopwindow   <- as.Date("2023-03-31", format ="%Y-%m-%d")
Cyclus_nr    <- 5
...
pat_nrs    <- read.csv("2304 0916 VBHC MACK Agenda afspraken.csv",header=T, sep=";", dec=",", stringsAsFactors = F, na.strings="", quote="", fileEncoding = "latin1") %>%
  ...
  filter(datum >= "2022-01-01" & datum <= "2022-12-31" &
         code %in% c("APNW", "PPC", "NP-19", "NPOLIB40")) %>% # 505 unieke patienten
  distinct(PatientNr, code)
table(pat_nrs$code) #alle patienten met APNW of PPC in 2022
...
length(unique(pat_nrs$PatientNr)) #n= 505 pt met APNW, PPC of NP-19 code.
length(unique(vp$PatientNr)) # n=209 met 318 voedselprovocatietesten

# patient selectie
pat_sel <- pat_nrs %>% filter(pat_nrs$PatientNr %in% vp$PatientNr) %>%
  distinct(PatientNr, .keep_all = TRUE)
length(unique(pat_sel$PatientNr)) #n = 209

#write.table(pat_sel$PatientNr, file = "./patientselectie_C5_2022_VP.csv", sep = ";", row.names = F) 

patsel_2021 <- read.csv("patientselectie_C5_2021_VP.csv",header=T, sep=";", dec=",", stringsAsFactors = F, na.strings="", quote="", fileEncoding = "latin1") %>%
  mutate(PatientNr = as.character(X.x.))
Overlap_patienten <- semi_join(pat_sel, patsel_2021, by = "PatientNr")
...
save(OutputDataSet, file = "OutputDataSetMACK_C5_2022.rda")
write.table(OutputDataSet, file = "OutputDataSet_2022.csv", sep = ";", row.names = F)
